import optuna
import yaml
import pipeline02_IMG_MLP as pipe
import mlflow
...
def objective(trial):
    ...
    tf.keras.backend.clear_session() #clear old tensorflow data to prevent memory exaustion due to previous runs
    ...
    #add callbacks for tracking and pruning
    cb_lst = []
    ...
    #open pipeline config and store new values
        ...
        #create model (try except block to catch errors for impossible models)
        try:
            #load model
...
#store best parameters to new config file
