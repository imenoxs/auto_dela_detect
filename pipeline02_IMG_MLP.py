...
def create_model(input_size, layers, neurons, dropout, kernel_init):
...
def main(cb_lst=[], model=None):
    config = dsutils.load_setup(os.path.join("configs","pipeline02config.yaml"))
    srcpath = config["Paths"]["srcpath"]

    batchsize = config["Hyperparameters"]["batch_size"]
    layers = config["Hyperparameters"]["layers"]
    neurons = config["Hyperparameters"]["neurons"]
    dropout= config["Hyperparameters"]["dropout"]
    kernel_init = config["Hyperparameters"]["kernel_init"]
    lr=config["Hyperparameters"]["lr_adam"]
    image_height = config["General"]["imagehight"]
    image_width = config["General"]["imagewidth"]
    image_size = (image_height, image_width)

    ds_train, ds_val = load_ds(os.path.join(srcpath,"data"), batch_size=batchsize, image_size=image_size)
    if model == None: model = create_model(input_size=image_size, layers=layers, neurons=neurons, dropout=dropout, kernel_init=kernel_init)
